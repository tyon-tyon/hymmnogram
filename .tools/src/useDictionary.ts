...
export default function () {
  // ローカルストレージから単語データを取得
  const words = { value: _words as TWord[] };
  ...
  */
  ...
  function getWordExactMatch(q: string, dialect?: string): TWord | undefined {
    ...
    if (founds.every(f => f.japanese.length === 0)) return undefined;
    if (founds.length) {
      // 単語が見つかった場合は複合語として返す
      return {
        hymmnos: q,
  ...
  function getWordEmotionVerb(q: string): TWord | undefined {
    ...
    for (const pastalieVerb of pastalieVerbs) {
      ...
      if (match) {
        ...
        if (emotionVowels.every(v => !v)) return undefined;
        // 想母音から意味を取得
  ...
  function getWordPossessive(q: string): TWord | undefined {
    ...
    if (!possessive) return undefined;
    ...
    if (!possessiveWord) return undefined;
